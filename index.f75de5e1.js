...
function sortTable(element) {
    if (element.tagName === 'TH') {
        ...
        const currentDirection = element.getAttribute('data-sort-direction') || 'desc';
        const newDirection = currentDirection === 'asc' ? 'desc' : 'asc';
        const lastSortedColumn = document.querySelector('[data-sort-direction]');
        if (lastSortedColumn && lastSortedColumn !== element) lastSortedColumn.removeAttribute('data-sort-direction');
        ...
        arr.forEach((obj)=>{
            ...
            Object.values(obj).forEach((data, index)=>{
                ...
                cell.textContent = index === 4 && typeof data === 'number' ? `$${data.toLocaleString('en-US')}` : data;
...
document.addEventListener('click', (ev)=>{
    ...
    else if (ev.target.tagName === 'TD') setActiveRow(ev.target);
});
createForm();
